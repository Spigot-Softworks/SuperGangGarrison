    ...
    private void ExecuteConsoleCommand(string commandText)
    {
        ...
        switch (command)
        {
            ...
            case "spawn_combat_dummy":
                ...
                if (parts.Length >= 2)
                {
                    if (!TryParsePlayerClass(parts[1], out var combatDummyClass))
                    {
                        AddConsoleLine("usage: spawn_combat_dummy [scout|engineer|pyro|soldier|demoman|heavy|sniper|medic|spy|quote]");
                        break;
                    }
