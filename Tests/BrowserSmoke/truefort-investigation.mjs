...
try {
  ...
  for (const map of maps) for (let repeat = 0; repeat < repeats; repeat++) {
    ...
    await page.evaluate(() => OpenGarrisonBrowserHost.runAutomationConsoleCommand('teleport 3800 666')); await page.evaluate(() => OpenGarrisonBrowserHost.runAutomationConsoleCommand('set_hp 10000')); await delay(16000);
    ...
    for (let step = 0; step < seconds; step += 10) { await delay(10000); await page.screenshot({path: join(output, 'truefort-' + step + '.png')}); }
